...
def update(table):
    ...
    def update_house():
        ...
        new_HType=st.number_input("Select the Type",Htype,disabled=True)
        new_Owner_ID=st.number_input("OwnerID",Owner_ID,disabled=True)
        if st.button("Submit"):
            c.execute("UPDATE House SET Rent_Cost=%s,House_Addr=%s WHERE "
              "House_ID=%s", (new_Rent,new_House_Addr, House_ID))
