...
def add_data_oph(Username,Ph_No):
    ...
    # c.execute('Select Owner_ID from Owner where Username="vrush41"')
    ids=c.fetchone()
    # return int(ids[0]) 
...
def add_data_tenant(F_Name,L_Name, Username, Password, Tenant_Addr,DOB):
    c.execute('INSERT INTO Owner(F_Name,L_Name, Username, Password, Tenant_Addr,DOB) VALUES (%s,%s,%s,%s,%s,%s)',
...
def delete_data_htype(id):
    c.execute("DELETE from House_type where Type={}".format(id[0]))
    mydb.commit()
