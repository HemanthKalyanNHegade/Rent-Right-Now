...
def newcontract(ten_id):
    ...
    if "Tenant_ID" in st.session_state:
        ...
        if st.button("Submit"):
            c.execute("INSERT into contract(Start_Date,Duration,Rent_Cost_Duration,Owner_Id,Tenant_Id,House_Id) values(%s,%s,%s,%s,%s,%s)",(Start_date,Duration,Rent_Cost_Duration,int(House_ID[2]),Tenant_ID,int(House_ID[0]) ))
...
def showconpay(ten_id):
    if "Tenant_ID" in st.session_state:
        ...
        for con in selected_con:
            ...
            c.execute("CALL end_date_calc({},{},@out);Select @out".format(start,duration))
            # c.execute("Select @out")
            show=c.fetchone()
            st.info("This contract ends on {}".format(show)) """
...
def showpayments(ten_id):
    ...
    if "Tenant_ID" in st.session_state:
        ...
        with st.expander("New Payment"):
            ...
            qrystr = ("""CREATE TRIGGER amount_exceed AFTER INSERT ON  Payment FOR EACH ROW BEGIN IF (select sum(Amount)+new.Amount from Payment group by Contract_ID={}) > {} THEN SIGNAL SQLSTATE '45000' SET message_text="Amount can't exceed total Rent Cost";END IF;END""".format(Contract_ID,totalrent))
            c.execute(qrystr)
            mydb.commit()
